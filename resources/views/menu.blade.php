...
<head>
  ...
  <style>
    ...
    .btn-box a {
      ...
      transition: background-color 0.3s;
      /* Transisi halus saat perubahan warna latar belakang */
    }

    .btn-box a:hover {
      background-color: #ffa500 !important;
      /* Warna hijau yang lebih gelap saat tombol di-hover */
    }
  </style>
        ...
        <div class="col-md-6 mx-auto">
          <form method="GET" action="{{ route('menu') }}">
            <div class="input-group">
              <input type="text" name="search" class="form-control" placeholder="Cari makanan..."
                value="{{ request('search') }}">
              <button class="btn btn-warning" type="submit">Cari</button>
              @if(request('search'))
                <a href="{{ route('menu') }}" class="btn btn-secondary">Reset</a>
              @endif
            </div>
          </form>
        </div>
      ...
      <div class="filters-content">
        <div class="row grid">
          @forelse ($makanan as $item)
            <div class="col-sm-6 col-lg-4 all {{ $item->daerah ? $item->daerah->nama : '' }}">
              <a href="{{ route('detail', ['id' => $item->id]) }}">
                <div class="box">
                  <div>
                    <div class="img-box">
                      <img src="{{ asset('images/' . $item->image) }}"
                           alt="{{ $item->nama }} - Makanan khas {{ $item->daerah ? $item->daerah->nama : 'Indonesia' }}"
                           loading="lazy">
                    </div>
          ...
          @empty
            <div class="col-12 text-center py-5">
              ...
              <h4>Tidak ada makanan ditemukan</h4>
              <p>Coba kata kunci lain atau <a href="{{ route('menu') }}">lihat semua menu</a></p>
            </div>
          @endforelse
        </div>
      </div>
